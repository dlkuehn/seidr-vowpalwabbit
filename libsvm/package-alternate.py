...
class Libsvm(MakefilePackage):
    ...
    def install(self, spec, prefix):
        ...
        if self.version < Version("330"):
            install("libsvm.so.2", prefix.lib)
        if self.version >= Version("330"):
            install("libsvm.so.3", prefix.lib)

    @run_after("install")
    def symlink_libsvm(self):
        with working_dir(self.prefix.lib):
            if self.version < Version("330"):
                symlink("libsvm.so.2", "libsvm.so")
            if self.version >= Version("330"):
                symlink("libsvm.so.3", "libsvm.so")
