...
class Liblinear(MakefilePackage):
    ...
    def install(self, spec, prefix):
        ...
        if self.version == Version("230"):
            ...
            install("liblinear.so.3", prefix.lib)
        if self.version > Version("230") and self.version < Version("244"):
            install("liblinear.so.4", prefix.lib)
        if self.version >= Version("244"):
            install("liblinear.so.5", prefix.lib)

    @run_after("install")
    def symlink_liblinear(self):
        with working_dir(self.prefix.lib):
            if self.version == Version("230"):
                symlink("liblinear.so.3", "liblinear.so")
            if self.version > Version("230") and self.version < Version("244"):
                symlink("liblinear.so.4", "liblinear.so")
            if self.version >= Version("244"):
                symlink("liblinear.so.5", "liblinear.so")
